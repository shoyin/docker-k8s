...
![image-20200929160643226](images/image-20200929160643226-1624853583498.png)



添加bin 环境变量 `path`

![image-20200929160823840](images/image-20200929160823840-1624853586073.png)
...
```go
fmt.Print(isOk)			在终端输出打印得内容
fmt.Printf("%v",isOk)	%s isok 变量占位符
fmt.Println(isOk)		打印指定内容追加换行符
fmt.Printf("%T\n", n)	类型
fmt.Printf("%v\n", n)	值
fmt.Printf("%b\n", n)	二进制
fmt.Printf("%d\n", n)	十进制
...
fmt.Print(5|2)  	111 7
fmt.Print(5^2)  	111 7
fmt.Print(5<<2)  	1010 10
fmt.Print(1<<10)  	100000000000 1024
fmt.Print(5>>1)  	10 2
...
举个例子，现在有一个数组`a := [8]int{0, 1, 2, 3, 4, 5, 6, 7}`，切片`s1 := a[:5]`，相应示意图如下。 ![slice_01](images/slice_01-1624853610622.png) 切片`s2 := a[3:6]`，相应示意图如下： ![slice_02](images/slice_02-1624853615872.png)
